...
### v1.1.0 — メッセージ操作の完全化（2026年3月予定）

**テーマ: 基本操作の強化**

- [ ] メッセージ編集 `ボット.メッセージ編集(チャンネルID, メッセージID, 新しい内容)`
- [ ] メッセージ削除 `ボット.メッセージ削除(チャンネルID, メッセージID)`
- [ ] メッセージ一括削除 `ボット.メッセージ一括削除(チャンネルID, 件数)`
- [ ] メッセージ取得 `ボット.メッセージ取得(チャンネルID, メッセージID)`
- [ ] メッセージ履歴 `ボット.メッセージ履歴(チャンネルID, 件数)`
- [ ] メッセージピン / ピン解除 `ボット.ピン(チャンネルID, メッセージID)`
- [ ] リプライ（参照付き） `ボット.リプライ(メッセージ, 内容)`
- [ ] リアクション削除 `ボット.リアクション削除(チャンネルID, メッセージID, 絵文字)`
- [ ] リアクション全削除 `ボット.リアクション全削除(チャンネルID, メッセージID)`
- [ ] タイムアウト `ボット.タイムアウト(サーバーID, ユーザーID, 秒数)`
- [ ] エラーイベント `ボット.エラー時(関数(エラー): ... 終わり)`
- [ ] 切断イベント `ボット.切断時(関数(): ... 終わり)`

**新規関数: +12**

---

### v1.2.0 — メッセージコンポーネント（2026年4月予定）

**テーマ: インタラクティブ UI**

- [ ] ボタン作成 `ボット.ボタン作成(ラベル, スタイル, カスタムID)`
- [ ] ボタン行作成 `ボット.アクション行作成()`
- [ ] ボタン行にボタン追加 `ボット.行にボタン追加(行, ボタン)`
- [ ] コンポーネント付きメッセージ送信 `ボット.コンポーネント送信(チャンネルID, テキスト, 行配列)`
- [ ] ボタンクリック時イベント `ボット.ボタン時(カスタムID, 関数(インタラクション): ... 終わり)`
- [ ] セレクトメニュー作成 `ボット.セレクトメニュー作成(カスタムID, プレースホルダー)`
- [ ] セレクトメニュー選択肢追加 `ボット.メニュー選択肢(メニュー, ラベル, 値, 説明)`
- [ ] セレクトメニュー選択時イベント `ボット.セレクト時(カスタムID, 関数(インタラクション): ... 終わり)`
- [ ] インタラクション更新 `ボット.インタラクション更新(インタラクション, 内容)`
- [ ] エフェメラル応答 `ボット.コマンド応答(インタラクション, 内容, 真)`（第3引数=エフェメラル）
- [ ] 遅延応答 `ボット.遅延応答(インタラクション)`
- [ ] フォローアップ `ボット.フォローアップ(インタラクション, 内容)`

**新規関数: +12 | 累計: ~79**

---

### v1.3.0 — モーダル & 高度なコマンド（2026年5月予定）

**テーマ: フォームとコマンド構造の充実**

- [ ] モーダル作成 `ボット.モーダル作成(カスタムID, タイトル)`
- [ ] テキスト入力追加 `ボット.テキスト入力追加(モーダル, ラベル, カスタムID, スタイル)`
- [ ] モーダル表示 `ボット.モーダル表示(インタラクション, モーダル)`
- [ ] モーダル送信時イベント `ボット.モーダル送信時(カスタムID, 関数(インタラクション): ... 終わり)`
- [ ] サブコマンド登録 `ボット.サブコマンド追加(コマンド名, サブ名, 説明, 関数)`
- [ ] サブコマンドグループ `ボット.サブコマンドグループ追加(コマンド名, グループ名, 説明)`
- [ ] オートコンプリート `ボット.オートコンプリート時(コマンド名, 関数(インタラクション): ... 終わり)`
- [ ] コンテキストメニュー（ユーザー） `ボット.ユーザーメニュー登録(名前, 関数)`
- [ ] コンテキストメニュー（メッセージ） `ボット.メッセージメニュー登録(名前, 関数)`
- [ ] コマンドオプション: 選択肢 `ボット.コマンド選択肢(オプション, 名前, 値)`
